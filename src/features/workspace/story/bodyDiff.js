...
// 백엔드가 주는 date는 "YYYY-MM-DD HH:MM"인데 **버전 탭에서는 시각을 뗀다.**
// 저자가 몇 시에 올렸는지는 "리뷰를 받고 무엇을 고쳤나"에 아무 것도 보태지 않는
// 반면, 탭의 폭을 정하는 것이 이 줄이라 대가는 크다 — 시각을 남기면 글씨를
// 13px로만 올려도 탭이 패널 밖으로 60px 넘쳐 가로 스크롤이 생긴다(실측, 패널
// 660px). 날짜만으로도 순서와 간격은 그대로 읽힌다.
// 본문 영역 헤더는 폭이 넉넉하므로 거기서는 시각을 그대로 둔다.
export const dateOnly = (date) => (date ?? '').split(' ')[0];
